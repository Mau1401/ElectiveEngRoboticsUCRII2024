...
dataObstaculo = readmatrix("DatosGlobales/obstaculo.csv");

% Extraer las columnas de coordenadas 
xObst = dataObstaculo(:,1);
yObst = dataObstaculo(:,2);
...
dataRobot = readmatrix('DatosRobot/braitenbergEquipo3.csv'); % Datos de la primera simulación
% = readmatrix('DatosGlobales/datos-globales-pp.csv'); % Datos de la segunda simulación
dataGlobal = readmatrix('DatosGlobales/datos-globales-brai.csv');
...
plot(xObst, yObst, 'r-', 'DisplayName', 'Obstaculo', 'LineWidth', 1);
hold on;
...
title('Comparativa de Trayectorias Braitenberg del Robot');
xlabel('Posición X (m)');
ylabel('Posición Y (m)');
legend('show');
grid on;
